    ...
    public partial class frmAcceso : Form
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string claveAcceso = Utilerias.Transform(this.tbAccess.Text);

                string acceso = ActualizarPrecios.Properties.Settings.Default.accesoConfig;

                if (Utilerias.Transform(acceso).Equals(Utilerias.Transform(claveAcceso)))
        ...
        private void btnPAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string claveAcceso = Modelo.Utilerias.Transform(this.tbPCveAnt.Text);

                string acceso = ActualizarPrecios.Properties.Settings.Default.accesoConfig;

                if (!Modelo.Utilerias.Transform(acceso).Equals(Modelo.Utilerias.Transform(claveAcceso)))
                {
                    this.ActiveControl = this.tbPCveAnt;
                ...
                string cveNAcceso = Modelo.Utilerias.Transform(this.tbPCveNueva.Text);

                ActualizarPrecios.Properties.Settings.Default.accesoConfig = cveNAcceso;

                ActualizarPrecios.Properties.Settings.Default.Save();
